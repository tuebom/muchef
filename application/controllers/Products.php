...
class Products extends Public_Controller {
	...
	public function index()
	{
		...
		if (strlen($kode) == 8)
		{
			$this->data['title']  = strtoupper($this->golongan3_model->get_by_id($kode)->nama);
		}
		else
		{
			$this->data['title']  = strtoupper($this->golongan2_model->get_by_id($kode)->nama);
		}
		
		$this->data['kdgol']  = substr($kode,0,2);
		$kdgol2 = substr($kode,0,5);
		
		$this->data['brands']   = $this->brands_model->get_by_id($kdgol2); //get_all();
		...
		$this->data['products']    = $this->stock_model->get_by_category(8, $offset, $kode); //, TRUE
		...
		if ($jml_master > 0)
		{
			$this->session->set_userdata('display_master', TRUE);
			// $this->data['products'] = $this->stock_model->get_by_category(8, $offset, $kode); //, TRUE
			// $this->data['pagination'] = $this->paging($jml_master, $page, $url);
		}
		else
		{
			$this->session->set_userdata('display_master', FALSE);
