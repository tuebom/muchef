...
class Checkout extends Public_Controller {
	...
	public function index()
	{
		...
		// if (empty($_POST)) {
			...
			// $origin      = 114; //denpasar
			// $destination = $_SESSION["regency"];
			
			// $result      = json_decode($this->rajaongkir->cost($origin, $destination, 1000, "jne"));
			// $cost_jne    = $result->rajaongkir->results[0]->costs;
			
			// $this->data['paket'] = $cost_jne;
			// die(print_r($cost_jne));

			// $cost_lion = $this->rajaongkir->cost($origin, $destination, 1000, "lion");
			// $cost_whn  = $this->rajaongkir->cost($origin, $destination, 1000, "wahana");
			// $cost_jnt  = $this->rajaongkir->cost($origin, $destination, 1000, "jnt");
		// }
		// else
		if (!empty($_POST)) {
	...
	public function hitung_ongkir()
	{
		...
		{
			$total_cost = $total_weight / 1000 * $cost;
		}
		...
		->set_status_header(200)
		->set_output(json_encode(array(
			'data' => array('shipcost' => $total_cost)
		)));
	}
}